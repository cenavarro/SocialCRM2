  ...
  def index
    if params.has_key?(:idc)
      if params[:opcion].to_i == 1
        ...
        @twitter_data = TwitterDatum.where(['start_date >= ? and end_date <= ? AND client_id = ?', fechaInicio,fechaFinal,params[:idc].to_i])
        @dates = ""
  ...
  def create
    ...
    respond_to do |format|
      if @twitter_datum.save
        format.html { redirect_to %{/twitter_data/#{@twitter_datum.client_id}/1}, notice: 'La informacion ha sido actualizada con exitosamente.'}
