class Monitoring < ActiveRecord::Base
  ...
  def self.generate_excel(document, social_id, start_date, end_date)
    @comments = MonitoringComment.find_by_social_network_id(social_id)
    themes = Monitoring.where('social_network_id = ? and isTheme = ?', social_id, true).order("name ASC")
    datum = get_monitoring_data(themes.first.id, start_date, end_date)
    channels = Monitoring.where('social_network_id = ? and isTheme = ?', social_id, false).order("name ASC")
    ...
    document.workbook do | wb |
      wb.add_worksheet(:name => "Monitoring Interno-Externo", :page_margins => margins, :page_setup => page_setup) do |sheet|
        ...
        add_images_monitoring_report(sheet, 123, social_id, styles)
      end
    end
  end
  ...
  def self.get_monitoring_data(id, start_date, end_date)
  ...
  def self.create_report_data(params ={})
  ...
  def self.create_theme_datum(params, monitoring_data)
  ...
  def self.create_channel_datum(params, monitoring_data)
  ...
  def self.add_table_monitoring(sheet, styles)
    add_rows_report(sheet, 2)
  ...
  def self.add_images_monitoring_report(document, y_pos, social_id, styles)
  ...
  def self.insert_themes_chart(sheet)
  ...
  def self.insert_channels_chart(sheet)
